...
\begin{enumerate}
    ...
    \item \textbf{Interface-specific fine-tuning} - Enhances the baseline model using additional interfacial configurations. This aims to reduce domain mismatch and improve ranking accuracy in challenging cases.

    \item \textbf{Delta-learning correction} - Learns the difference between MLP and DFT energy outputs (\Delta E = E_\mathrm{DFT} - E_\mathrm{MLP}). This approach enables low-cost calibration without full retraining, making it attractive for incremental improvements.
...
\begin{itemize}
    ...
    \item \textbf{Kendall's} (\( \tau \)): Assesses the proportion of concordant vs. discordant ranking pairs. It is more sensitive to small local swaps in ranking than Spearman's \rho.
    \item \textbf{Top-N Overlap}: Evaluates how many of the top-ranked structures under DFT are also present in the MLP top-N list. This provides a practical indicator for screening pipelines.
    \item \textbf{Mean Absolute Error (MAE) and Root Mean Square Error (RMSE)}: These absolute error metrics help assess whether large deviations occur, even if ranking is preserved.
...
A figure is planned to summarise rank correlation across these groups, e.g., from files like \texttt{results\_lower\_Ge\_upper\_Sn\_kendall\_dft@dft\_vs\_mlp@mlp\_valid.png}.
...
% TODO: Add grouped figure comparing ranking metrics across symmetry and orientation

\section{Integration of RAFFLE into Structure Generation}

While ARTEMIS enables systematic construction of aligned interface stacks with controlled Miller plane matching and registry shifts, its deterministic approach limits structural diversity. This can lead to overrepresentation of high-symmetry configurations and under-sampling of disordered or experimentally relevant interfaces.

RAFFLE addresses this limitation by introducing stochastic perturbations within the interfacial zone. Using the \texttt{raffle\_generator} module, RAFFLE will:
\begin{itemize}
