...
\newpage Matching 22 unit cells of Si to 21 of Ge gives:
\begin{equation}
...
\newpage While ARTEMIS considered many potential surface combinations, only $29$ unique surfaces were ultimately included in the final dataset.
...
\newpage \subsection{Relaxation and Energy Evaluation}
...
\newpage Across all evaluated systems, Spearman's rank correlation coefficient was extremely high:
\begin{equation}
...
\newpage \subsection*{Silicon Systems $(Si|X)$}
...
\newpage \subsection*{Germanium Systems}
...
\newpage \paragraph{Tin Systems} $Sn$ systems performed significantly better across all metrics:
...
\newpage \subsection*{Grain Boundaries (Homostructures)}
...
\newpage \subsection*{Perfect rank preservation}
Perfectly intermixed alloy systems exhibited near-perfect rank alignment between DFT and MLP evaluations~\nocite{}.
...
\newpage These observations highlight the importance of geometric configuration in model reliability.
...
\newpage Carbon results were geometry-dependent: only three valid $C|Si$ structures with carbon in the lower slab were obtained, and these yielded unreliable results ($\rho = -0.5$), leading to their exclusion from analysis.
