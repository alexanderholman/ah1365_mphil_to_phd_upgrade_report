...
energetically reasonable heterostructures\cite{Di_Liberto2022-gz, Batzner2022-fd}.

Ideally, such a dataset would be generated entirely using density functional theory (DFT), but the cost of relaxing and
evaluating thousands of configurations makes this impractical\cite{Rosen2022-pw, Schutt2018-xc, Rosen2022-pw}.
...
Elements heavier than atomic number 83 are excluded to avoid needing relativistic corrections, it is also notable that
MACE is trained on elements $Z \le 89$.
...
Interfaces were generated using the ARTEMIS toolkit (see \secref{sec:constructing_interfaces} for background on
interface construction and lattice matching).
...
directions, looking for configurations with similar lattice vectors and angles (detailed in \secref{sec:lattice_mismatching}).
...
unbonded valence electrons\cite{Gerber2023-yd, Butler2019-ws}.

Interfaces with large lattice mismatch or poor atomic registry often exhibit elevated formation energies due to elastic
distortions or chemically unstable bonding environments\cite{Hepplestone2010-vw, Chagarov2015-ol, Zhang2016-lt}.

By contrast, coherent interfaces that exhibit low strain and high atomic registry are more likely to be energetically
stable and experimentally realisable\cite{Gerber2023-yd, Davies2024-ju, Chagarov2015-ol}.

% ---------- %

One strategy for achieving this is through supercell scaling\cite{artemis_paper}: we try to match lattice constants $a$ and $a'$ of two
...
unit cells (see discussion of supercell mismatch reduction in \secref{sec:lattice_mismatching}).
...
In addition to supercell scaling, the choice of interface orientation, described by Miller indices (explained in
\secref{sec:orientation_matching}), can significantly influence atomic registry across the interface.
...
across the interface is maximised and valence electrons are not left unpaired\cite{artemis_paper}.
...
(details of which are discussed in \secref{sec:artemis_algorithm}).
...
Although ARTEMIS supports atomic swaps at the interface to explore alloy-like or disordered bonding environments\cite{artemis_paper}, this
...
These metrics help to determine whether the MLP is giving similar energy rankings and values to DFT, so it can be established
...
To understand how interface geometry impacts ranking fidelity, results were grouped into three categories: upper-slab
...
% MACE, particularly under strain or poor coordination. $ remove?
...
\begin{table}[h]
    \centering
    \caption{Performance metrics for Si interfaces (Si in upper slab). Lower-slab Si configurations exhibited even poorer rank correlation (not shown), highlighting geometry sensitivity.}
...
Although direct MLP-on-DFT-relaxed structure evaluations (DFT relacex MLP) were not included due to computational cost, the
study revealed that relaxation, evaluation consistency (e.g. full MLP vs full DFT) influences error, and future
