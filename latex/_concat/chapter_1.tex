...
They can reduce thermal conductivity compared to bulk values; an effect exploitable in thermoelectric applications~\cite{flores2012_interfaces}.
...
In systems like $\ce{CaCu3Ti4O12}$, colossal permittivity arises not from the bulk crystal but from insulating grain boundaries and a semiconducting interior, exemplifying the Maxwell-Wagner effect~\cite{Taylor2019-cpmech}.
...
\label{thermal_transport_interfaces} Thermal transport across interfaces is typically suppressed due to phonon scattering, mass disorder, and acoustic impedance mismatch~\cite{Liu2025-phonon}.
...
\label{interface_stabilise_metastable_phases} Interfaces can stabilise new or metastable phases not accessible in bulk~\cite{raffle_paper, Schusteritsch2015-lw}.
For instance, the interface between $\ce{BaTiO3}$ and $\ce{SiO2}$ has been shown to promote the spontaneous formation of the fresnoite phase $\ce{Ba2TiSi2O8}$, altering both electronic and mechanical response.
...
However, experimental and theoretical studies suggest that a mixed or hybrid phase may also emerge with phase stability dependent on thickness and interface strain~\cite{Pitfield2024-ra}.
...
\newpage \label{objectives_dft_validation} Once promising interfaces are identified, a subset will be fully relaxed using DFT to provide high-fidelity data for MLI training~\nocite{}.
...
\newpage \label{objectives_mlp_refinement} To improve the accuracy of interface energy predictions, particularly for systems where MACE performs poorly (e.g. strained silicon-rich structures or cross-material environments), this project will explore refinement strategies for the MLP itself~\nocite{}.
...
\newpage \label{objectives_failure_diagnostics} This work will systematically assess when and why these failures occur, starting with Group IV interfaces as test cases.
...
Where needed, refinement strategies such as delta-learning, fine-tuning, and active learning will be deployed to correct these behaviours~\cite{Pitfield2025-ah}.
...
\label{objectives_structure_insight} In parallel with predictive modelling, the project seeks to uncover the structural and chemical features that determine interfacial stability~\nocite{}.
...
\label{objectives_impact} By integrating machine learning (ML) with crystallographic modelling and ab initio validation, this work supports the development of practical tools for scalable, data-driven interface discovery in electronic, optoelectronic, and catalytic materials~\nocite{}.
...
\label{mlp_workflow_integration} Machine learning models, particularly MLPs, provide a way forward~\nocite{}.
