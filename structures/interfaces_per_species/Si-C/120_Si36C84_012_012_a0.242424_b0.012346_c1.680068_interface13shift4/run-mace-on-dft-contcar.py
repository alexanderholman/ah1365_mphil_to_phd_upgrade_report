...
if __name__ == "__main__":
    ...
    if not os.path.exists(poscar_file):
        print("CONTCAR file not found in the current directory.")
    else:
        energy = run_mace_calculation(poscar_file)
        with open("OUTCAR_DFT_MLP_MACE", "w") as f:
            f.write(f"MACE Energy: {energy:.6f} eV\n")

        print(f"MACE calculation complete. Energy written to OUTCAR_DFT_MLP_MACE: {energy:.6f} eV")
