...
run_base_dir = base_dir / "DRUN_DFT_VASP"
...
def find_and_run_sbatch(path: Path):
    for root, dirs, files in os.walk(path):
        root_path = Path(root)
        if "POSCAR" in files:
            has_output = any(f.endswith("DFTVasp.out") for f in files)
            if not has_output:
                sbatch_file = root_path / "SBATCH_DFT_VASP"
                if sbatch_file.exists():
                    print(f"Submitting job in {root_path}")
                    try:
                        result = subprocess.run(["sbatch", "SBATCH_DFT_VASP"], cwd=root_path, capture_output=True, text=True)
